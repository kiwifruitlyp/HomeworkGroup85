import time

# AES算法中使用的S盒
s_box = [
        ,
	];


# AES算法中使用的固定轮常量
r_con = [
    [0x01, 0x00, 0x00, 0x00], [0x02, 0x00, 0x00, 0x00], [0x04, 0x00, 0x00, 0x00], [0x08, 0x00, 0x00, 0x00],
    [0x10, 0x00, 0x00, 0x00], [0x20, 0x00, 0x00, 0x00], [0x40, 0x00, 0x00, 0x00], [0x80, 0x00, 0x00, 0x00],
    [0x1b, 0x00, 0x00, 0x00], [0x36, 0x00, 0x00, 0x00]]

# 字节替换
def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = s_box[state[i][j]]

# 行迁移
def shift_rows(state):
    for i in range(1, 4):
        state[i] = state[i][i:] + state[i][:i]

# 列混淆
def mix_columns(state):
    for i in range(4):
        column = state[i]
        temp = [0] * 4
        temp[0] = multiply(column[0], 2) ^ multiply(column[3], 1) ^ multiply(column[2], 1) ^ multiply(column[1], 3)
        temp[1] = multiply(column[1], 2) ^ multiply(column[0], 1) ^ multiply(column[3], 1) ^ multiply(column[2], 3)
        temp[2] = multiply(column[2], 2) ^ multiply(column[1], 1) ^ multiply(column[0], 1) ^ multiply(column[3], 3)
        temp[3] = multiply(column[3], 2) ^ multiply(column[2], 1) ^ multiply(column[1], 1) ^ multiply(column[0], 3)
        state[i] = temp

# 加轮密钥
def add_round_key(state, key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= key[i][j]

# 字节乘法
def multiply(a, b):
    p = 0
    for i in range(8):
        if b & 1:
            p ^= a
        hi_bit_set = a & 0x80
        a <<= 1
        if hi_bit_set:
            a ^= 0x1b
        b >>= 1
    return p % 256

# 生成轮密钥
def generate_round_keys(master_key):
    round_keys = [[0] * 4 for _ in range(44)]
    for i in range(4):
        for j in range(4):
            round_keys[j][i] = master_key[i * 4 + j]
    
    for i in range(4, 44):
        temp = round_keys[i - 1]
        if i % 4 == 0:
            temp = sub_word(rot_word(temp))
            temp[0] ^= r_con[i // 4 - 1][0]
            temp[1] ^= r_con[i // 4 - 1][1]
            temp[2] ^= r_con[i // 4 - 1][2]
            temp[3] ^= r_con[i // 4 - 1][3]
        round_keys[i] = [round_keys[i - 4][j] ^ temp[j] for j in range(4)]
    
    return round_keys

# 字节替换
def sub_word(word):
    return [s_box[b] for b in word]

# 行迁移
def rot_word(word):
    return word[1:] + [word[0]]

# 加密
def encrypt(plaintext, master_key):
    # 将明文分块并转换为状态矩阵
    state = [[plaintext[i + 4 * j] for i in range(4)] for j in range(4)]
    
    # 获取轮密钥
    round_keys = generate_round_keys(master_key)
    
    # 轮函数
    add_round_key(state, round_keys[:4])
    for i in range(1, 10):
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        add_round_key(state, round_keys[4 * i : 4 * (i + 1)])
    sub_bytes(state)
    shift_rows(state)
    add_round_key(state, round_keys[40:])
    
    # 将状态矩阵转换回密文
    ciphertext = [state[i][j] for j in range(4) for i in range(4)]
    return ciphertext

# 测试程序运行时间
start_time = time.perf_counter()

# 测试数据
plaintext = [0x54, 0x68, 0x69, 0x73, 0x20, 0x69, 0x73, 0x20, 0x61, 0x20, 0x74, 0x65, 0x73, 0x74, 0x20, 0x6d]
master_key = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c]

# 加密
ciphertext = encrypt(plaintext, master_key)

end_time = time.perf_counter()

# 输出结果和运行时间
print('明文:', [hex(p) for p in plaintext])
print('密钥:', [hex(k) for k in master_key])
print('密文:', [hex(c) for c in ciphertext])
print('程序运行时间:', end_time - start_time, '秒')
